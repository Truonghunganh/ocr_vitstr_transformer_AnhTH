...
class AttnLabelConverter(object):
    ...
    def encode(self, text, batch_max_length=25):
        ...
        # print("hhhhhhhhhhhhhhh",len(text),text)
...
def get_args(is_train=True):
    ...
    parser.add_argument('--checkExistHW', help='the height of the input image')
    parser.add_argument('--imgH', type=int, default=32, help='the height of the input image')
    parser.add_argument('--imgW', type=int, default=100, help='the width of the input image')
    parser.add_argument('--rgb',default=False, action='store_true', help='use rgb input')
    parser.add_argument('--character', type=str,default=get_data_txt("character.txt"), help='character label')
    ...
    parser.add_argument('--TransformerAnhTH',default=False, action='store_true', help='Use end-to-end transformer')

    choices = ["vitstr_tiny_patch16_224", "vitstr_small_patch16_224", "vitstr_base_patch16_224","vitstr_base_patch16_224_aug", "vitstr_tiny_distilled_patch16_224", "vitstr_small_distilled_patch16_224"]
    ...
    return args
'''
khi  co chu in hoa thi dung tat ca cac character
'''
def get_argsAnhTH(is_train=True):
    # AnhTH_train_data="data_train/bienso/BiensoxeVN_full_AnhTH/db_train/"
    # AnhTH_valid_data="data_train/bienso/BiensoxeVN_full_AnhTH/db_train/"
    AnhTH_train_data="/home/vbpo/Desktop/AnhTH/OCR/ocr_vitstr_transformer_AnhTH/data_train/SGC/HD/Class08_V1/db_train/"
    AnhTH_valid_data="/home/vbpo/Desktop/AnhTH/OCR/ocr_vitstr_transformer_AnhTH/data_train/SGC/HD/Class08_V1/db_train/"
    # AnhTH_train_data='data_train/YOBE/Diem/db_train/'
    # AnhTH_valid_data='data_train/YOBE/Diem/db_train/'
    AnhTH_batch_size=1
    AnhTH_num_iter=500000
    AnhTH_valInterval=2
    AnhTH_imgH=22
    AnhTH_imgW=22
    AnhTH_rgb=False
    # AnhTH_character="30f517968eb24zgpkatxuvhdslncmyro"
    # AnhTH_character="$0123456789abcdefghiklmnpqrstuvxyz"
    # AnhTH_character="0123456789"

    AnhTH_character=string.printable[:-6]
    # listchar=sorted(AnhTH_character)
    # AnhTH_character=''
    # for i in listchar:
    #     AnhTH_character+=i
    AnhTH_sensitive=True
    AnhTH_batch_max_length=100
    # AnhTH_saved_model='OCR/YOBE/ocr_roatienza_YOBE_Diem/ocr_roatienza_YOBE_Diem/best_accuracy.pth'
    AnhTH_saved_model=""
    AnhTH_Transformer=True
    parser = argparse.ArgumentParser(description='STR')
    ...
    # for train
    parser.add_argument('--exp_name',help='Where to store logs and models')
    parser.add_argument('--train_data', default=AnhTH_train_data, help='path to training dataset')
    parser.add_argument('--valid_data', default=AnhTH_valid_data, help='path to validation dataset')
    parser.add_argument('--manualSeed', type=int, help='for random seed setting')
    parser.add_argument('--workers', type=int, help='number of data loading workers. Use -1 to use all cores.', default=4)
    parser.add_argument('--batch_size', type=int, default=AnhTH_batch_size, help='input batch size')
    parser.add_argument('--num_iter', type=int, default=AnhTH_num_iter, help='number of iterations to train for')
    parser.add_argument('--valInterval', type=int, default=AnhTH_valInterval, help='Interval between each validation')
    parser.add_argument('--saved_model', default=AnhTH_saved_model, help="path to model to continue training")
    parser.add_argument('--FT', action='store_true', help='whether to do fine-tuning')
    parser.add_argument('--sgd', action='store_true', help='Whether to use SGD (default is Adadelta)')
    parser.add_argument('--adam', action='store_true', help='Whether to use adam (default is Adadelta)')
    parser.add_argument('--lr', type=float, default=1, help='learning rate, default=1.0 for Adadelta')
    ...
    parser.add_argument('--total_data_usage_ratio', type=str, default='1.0',
                        help='total data usage ratio, this ratio is multiplied to total number of data.')
    parser.add_argument('--batch_max_length', type=int, default=AnhTH_batch_max_length, help='maximum-label-length')
    parser.add_argument('--checkExistHW', help='the height of the input image')
    parser.add_argument('--imgH', type=int, default=AnhTH_imgH, help='the height of the input image')
    parser.add_argument('--imgW', type=int, default=AnhTH_imgW, help='the width of the input image')
    parser.add_argument('--rgb',default=AnhTH_rgb, action='store_true', help='use rgb input')
    # parser.add_argument('--character', type=str,default="51A027F49G836T$CHNVBMEDLPZKUSYRXIkO", help='character label')
    parser.add_argument('--character', type=str,default=AnhTH_character, help='character label')
    parser.add_argument('--sensitive',default=AnhTH_sensitive, action='store_true', help='for sensitive character mode')
    parser.add_argument('--PAD', action='store_true', help='whether to keep ratio then pad for image resize')
    parser.add_argument('--data_filtering_off', action='store_true', help='for data_filtering_off mode')
    
    """ Model Architecture """
    parser.add_argument('--Transformer',default=AnhTH_Transformer, action='store_true', help='Use end-to-end transformer')
    parser.add_argument('--TransformerAnhTH',default=False, action='store_true', help='Use end-to-end transformer')
    ...
    # parser.add_argument('--Prediction', type=str,default="Attn", help='Prediction stage. None|CTC|Attn')
